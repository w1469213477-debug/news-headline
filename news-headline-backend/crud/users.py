...
from schemas.users import UsersRequest, UserUpdateRequest
...
# 创建用户
async def create_user(db: AsyncSession, user_data: UsersRequest):
    # 先加密后处理
    ...
    db.add(user)
    await db.commit()
    await db.refresh(user) # 从数据库返回最新的user
    return user

async def create_token(db: AsyncSession, user_id: int):
    # 生成 Token + 设置过期时间 -> 查询数据库当前用户是否有 Token -> 有：更新；没有添加
